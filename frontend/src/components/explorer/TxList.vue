    ...
    <div class="row tx-info" v-for="item in transactions" :key="item.id">
      ...
      <div class="col-md-4">
        <router-link
          ...
          class="tx-number"
          >{{ $filters.truncate(item.hash, 10) }}</router-link
        >
      ...
      <div class="col-md-6 addresses">
        <p>
          ...
          {{ $filters.truncate(item.from, 10) }}
        </p>
        <p>
          <label class="text-secondary">To</label>
          {{ $filters.truncate(item.to, 10) }}
        </p>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async initTxes(height) {
      ...
      let block;
      while (height >= 0 && this.transactions.length < this.countToShow) {
