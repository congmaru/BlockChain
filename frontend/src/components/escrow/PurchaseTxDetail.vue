...
export default {
  ...
  mounted: function() {
    ...
    this.tx = vm.$store.state.purchaseTransactions.find(tx => tx.purchaseId == purchaseId);
    if (!this.tx) {
      throw new Error(`$store.state.purchaseTransactions에 purchaseId가 없음/purchaseId: ${purchaseId}`);
    }
    this.setActionText();
    // 내 지갑 정보 조회
    ...
    findUserById(
      ...
      function(res) {
        ...
        vm.tx.sellerName = name;
        vm.tx.sellerEmail = email;
      },
      function(err) {
