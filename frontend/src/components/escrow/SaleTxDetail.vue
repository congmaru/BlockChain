...
export default {
  ...
  mounted: function() {
    ...
    this.tx = vm.$store.state.saleTransactions.find(tx => tx.purchaseId == purchaseId);
    if (!this.tx) {
      throw new Error(`$store.state.saleTansactions에 purchaseId가 없음/purchaseId: ${purchaseId}`);
    }
    // 내 지갑 정보 조회
    ...
    findUserById(
      this.tx.buyerId,
      function(res) {
        vm.tx.buyerName = res.data.name;
        vm.tx.buyerEmail = res.data.email;
      },
      function(err) {
