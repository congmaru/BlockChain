          ...
          <div class="card">
            ...
            <div class="card-body">
              <div class="form-group">
                <div class="row">
                  <img :src="getImg(item?.image)" class="col-3" />
              ...
              <div class="form-group">
                ...
                <div v-else>
                  ...
                  <p>
                    ...
                    <span
                      v-else-if="
                        Number(wallet.cash) < Number(item?.price) + 20
                      "
...
export default {
  ...
  methods: {
    purchase() {
      if (this.purchasing) return; // 중복 클릭 방지
  ...
  computed: {
    canPurchase() {
      return (
        ...
        this.item.price <= this.wallet.cash
      );
    }
  },
  mounted: function() {
    const vm = this;
    vm.item = vm.$store.state.purchasingItem;
    if (this.$route.params.id != vm.item.id) {
      console.error('매개변수로 전달한 상품 id와 vuex의 purchasingItem.id가 다름');
    }
    // 내 지갑 정보 조회
